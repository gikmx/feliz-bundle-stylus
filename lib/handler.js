...
module.exports = function (file$, request, reply, options){
    ...
    const bundle_file$ = file$
        ...
        .mergeMap(file => Rx.Observable.create(observer => {
            ...
            if (!this.util.is(options.path).array()) options.path = [this.path.root];

            let stylus = Stylus(file.body.toString('utf8'))
                .set('filename', file.path);

            // Determine paths according to options
            options.path.forEach(path => stylus = stylus.include(path));

            // Apply engine options (if any)
